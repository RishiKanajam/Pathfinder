...
# ── Per-program specialty signal profiles ────────────────────────────────────
# signals: list of (keyword, weight) — higher weight = stronger indicator
# risk_fit: how well the program suits each risk level (0–1)
# emotion_fit: which emotions this program is designed to address (0–1)
...
def _matches_keyword(text: str, keyword: str) -> bool:
    """Match keyword in text. Short words (<=4 chars) require word boundaries to avoid false positives."""
    if len(keyword) <= 4:
...
def _signal_score(text: str, signals: list[tuple[str, int]]) -> tuple[float, list[str]]:
    """Return (0–1 score, list of matched keywords)."""
    lower = text.lower()
    matched: list[str] = []
    total_weight = 0
    for keyword, weight in signals:
        if _matches_keyword(lower, keyword):
            ...
            total_weight += weight
    # Normalise against the top-5 signal weights so a single strong hit = ~1.0
...
def match_programs(
    ...
    for program in programs:
        ...
        # Signal 1 — Specialty keyword match (45%)
        sig_score, matched_kws = _signal_score(situation, signals)

        # Signal 2 — Risk level alignment (25%)
        risk_score = risk_fit.get(risk_level, 0.5)

        # Signal 3 — Emotion alignment (15%)
        emotion_score = 0.0
        ...
        # Map to 0.35–0.97 so scores feel credible, not inflated
