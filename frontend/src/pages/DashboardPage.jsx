...
export default function DashboardPage({ activeStaff }) {
  ...
  const myCases = useMemo(
    () => referrals
      .filter(r => r.assigned_to === activeStaff?.id)
  ...
  return (
    <section className="dashboard">
      ...
      <div className="dashboard-head">
        <div>
          ...
          <p style={{ color: "var(--muted)", fontSize: "0.88rem", margin: "0.25rem 0 0" }}>
            ...
            {myCases.length > 0 && (
              <span style={{ marginLeft: "0.75rem", color: "var(--green-deep)", fontWeight: 700 }}>
                · {myCases.length} assigned to you
              </span>
            )}
          </p>
        </div>
      ...
      {/* ── My cases ── */}
      {myCases.length > 0 && (
        <div style={{ padding: "0 1.5rem", marginBottom: "0" }}>
          <section className="wide-panel" style={{ marginTop: "1rem" }}>
            <div className="section-heading">
              <UserRound size={18} />
              <h3>My cases — {myCases.length} assigned to {activeStaff?.name || "you"}</h3>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "0.75rem" }}>
              {myCases.slice(0, 4).map((ref, idx) => {
                const riskBorderColor = { low: "var(--risk-low)", medium: "var(--risk-medium)", high: "var(--risk-high)", critical: "var(--risk-critical)" }[ref.risk_level] || "var(--green-sage)";
                return (
                  <article
                    key={ref.id}
                    className="referral-card"
                    style={{ animationDelay: `${idx * 40}ms`, borderLeftColor: riskBorderColor, margin: 0 }}
                  ...
                  >
                    ...
                    <p>{ref.situation}</p>
                    <small>{riskLabel[ref.risk_level]} risk · {timeAgo(ref.created_at)}</small>
                  </article>
                );
              })}
            </div>
            {myCases.length > 4 && (
              <p style={{ fontSize: "0.8rem", color: "var(--muted)", marginTop: "0.75rem" }}>
                +{myCases.length - 4} more in the pipeline below
              </p>
            )}
          </section>
        </div>
      ...
      <div className="dashboard-grid">
                ...
                {activeReferrals.map((ref, idx) => {
                  ...
                  return (
                    <article
                      ...
                      key={ref.id}
                      style={{ animationDelay: `${idx * 40}ms`, borderLeftColor: riskBorderColor }}
                      onClick={() => openReferral(ref, "detail")}
                    >
                      <div className="referral-card-top">
                        <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
        ...
        {/* Risk queue */}
        {profile.showRiskQueue && <section className="side-panel" style={{ gridColumn: "span 2" }}>
          <div className="section-heading">
            <BellRing size={18} />
        ...
        {/* Escalation status */}
        {profile.showEscalation && <section className="side-panel" style={{ gridColumn: "span 2" }}>
          <div className="section-heading">
        ...
        {/* Volunteer roster */}
        {profile.showVolunteers && <section className="side-panel" style={{ gridColumn: "span 2" }}>
          <div className="section-heading">
            <UsersRound size={18} />
        ...
        {/* Staff management — CEO / delegated only */}
        {staff.find(s => s.id === activeStaff?.id)?.can_manage_staff && (
          <section className="wide-panel" style={{ gridColumn: "span 4" }}>
            <StaffManagementPanel
              staff={staff}
              activeStaff={activeStaff}
              onStaffUpdated={(updated) => setStaff(prev => prev.map(s => s.id === updated.id ? updated : s))}
              onStaffCreated={(member) => setStaff(prev => [...prev, member])}
            />
          </section>
        )}
      </div>
...
function MetricCard({ label, value, detail, accent = false }) {
  return (
    <article
      ...
      style={accent ? { borderColor: "rgba(224,122,95,0.3)", borderTopColor: "var(--coral)" } : {}}
    >
      <span>{label}</span>
...
const PERMISSION_LABELS = {
  can_manage_staff:    { label: "Manage staff",       description: "Can create new staff IDs and change permissions" },
  can_handle_high_risk:{ label: "High-risk cases",    description: "Assigned to high-risk and crisis referrals" },
  is_on_call:          { label: "On call",            description: "Currently on the on-call roster" },
};

const EMPTY_FORM = { name: "", role: "", email: "", can_handle_high_risk: false, is_on_call: false, max_caseload: 15 };

function StaffManagementPanel({ staff, activeStaff, onStaffUpdated, onStaffCreated }) {
  ...
  async function handleCreate(e) {
    ...
    try {
      const member = await api.createStaff({ ...form, requested_by: activeStaff.id });
      onStaffCreated(member);
      setSuccess(`Staff ID #${member.id} created for ${member.name}.`);
      setForm(EMPTY_FORM);
      setShowForm(false);
    } catch {
      setError("Failed to create staff member. Check your permissions.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function togglePermission(member, permission) {
    ...
    try {
      const updated = await api.grantPermission(member.id, {
        permission, value: newValue, granted_by: activeStaff.id,
      });
      onStaffUpdated(updated);
    } catch {
      // silently ignore — UI will not update
    }
  }
  ...
  return (
    <>
      ...
      {showForm && (
        <form onSubmit={handleCreate} style={{ display: "grid", gap: "0.75rem", marginBottom: "1.25rem", padding: "1rem", background: "var(--bg-warm)", borderRadius: "var(--radius-md)", border: "1px solid var(--border)" }}>
          <p style={{ margin: 0, fontWeight: 700, fontSize: "0.9rem", color: "var(--charcoal)" }}>Create new staff member</p>
          <div className="pro-grid">
            <label style={{ display: "flex", flexDirection: "column", gap: "0.3rem", fontSize: "0.85rem", fontWeight: 600 }}>
              Full name *
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Jordan Smith" style={{ border: "1.5px solid var(--border-mid)", borderRadius: "var(--radius-md)", padding: "0.6rem 0.8rem", background: "var(--bg-card)", outline: "none" }} />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.3rem", fontSize: "0.85rem", fontWeight: 600 }}>
              Role *
              <input value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))} placeholder="e.g. Support worker" style={{ border: "1.5px solid var(--border-mid)", borderRadius: "var(--radius-md)", padding: "0.6rem 0.8rem", background: "var(--bg-card)", outline: "none" }} />
            </label>
          </div>
          <label style={{ display: "flex", flexDirection: "column", gap: "0.3rem", fontSize: "0.85rem", fontWeight: 600 }}>
            Email
            <input value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="name@lmnspn.org.au" type="email" style={{ border: "1.5px solid var(--border-mid)", borderRadius: "var(--radius-md)", padding: "0.6rem 0.8rem", background: "var(--bg-card)", outline: "none" }} />
          </label>
          <div style={{ display: "flex", gap: "1.5rem", fontSize: "0.85rem" }}>
            <label style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontWeight: 600, cursor: "pointer" }}>
              <input type="checkbox" checked={form.can_handle_high_risk} onChange={e => setForm(f => ({ ...f, can_handle_high_risk: e.target.checked }))} />
              High-risk capable
            </label>
            <label style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontWeight: 600, cursor: "pointer" }}>
              <input type="checkbox" checked={form.is_on_call} onChange={e => setForm(f => ({ ...f, is_on_call: e.target.checked }))} />
              On call
            </label>
          </div>
          {error && <p style={{ color: "var(--coral)", fontSize: "0.82rem", margin: 0 }}>{error}</p>}
          {success && <p style={{ color: "var(--green-deep)", fontSize: "0.82rem", margin: 0, fontWeight: 700 }}>{success}</p>}
          <div style={{ display: "flex", gap: "0.6rem" }}>
            <button type="submit" className="primary-button" disabled={saving} style={{ minWidth: 140 }}>
              <Plus size={15} /> {saving ? "Creating…" : "Create staff ID"}
            </button>
            <button type="button" className="quiet-button" onClick={() => { setShowForm(false); setError(""); }}>Cancel</button>
          </div>
        </form>
      )}

      {success && !showForm && (
        <p style={{ color: "var(--green-deep)", fontSize: "0.82rem", marginBottom: "0.75rem", fontWeight: 700 }}>{success}</p>
      )}

      <div style={{ display: "grid", gap: "0.6rem" }}>
        {staff.map(member => (
          <div key={member.id} style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "0.75rem 0.85rem", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", minWidth: 180 }}>
              <span className="staff-initials" style={{ background: member.can_manage_staff ? "var(--green-deep)" : "var(--green-mid)" }}>
                {member.name.split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase()}
              </span>
              <div>
                <strong style={{ display: "block", fontSize: "0.88rem", color: "var(--charcoal)" }}>{member.name}</strong>
                <small style={{ color: "var(--muted)", fontSize: "0.75rem" }}>ID #{member.id} · {member.role}</small>
              </div>
            </div>
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", flex: 1 }}>
              {Object.entries(PERMISSION_LABELS).map(([perm, meta]) => {
                const active = !!member[perm];
                const isSelf = member.id === activeStaff?.id && perm === "can_manage_staff";
                return (
                  <button
                    key={perm}
                    ...
                    style={{
                      ...
                      fontSize: "0.75rem", fontWeight: 700, cursor: isSelf ? "default" : "pointer",
                      border: `1.5px solid ${active ? "rgba(45,106,79,0.3)" : "var(--border-mid)"}`,
                      background: active ? "var(--green-pale)" : "var(--bg-card)",
